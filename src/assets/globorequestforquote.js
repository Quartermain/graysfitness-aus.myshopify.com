...
//         if(GRFQConfigs.show_price=='2') $(GRFQConfigs.selector.price_selector).attr('style','display:initial !important');
        ...
        if(GRFQApp.getPageType() == 'collection' || GRFQApp.getPageType() == 'product' || GRFQApp.getPageType() == 'search' || GRFQApp.getPageType() == '' ){
        ...
        if(GRFQApp.getPageType() == 'product'){
          if(GRFQConfigs.shop_url=="mueblesysillones.myshopify.com") if(GRFQConfigs.show_atc=='0' || GRFQConfigs.show_atc=='1') $(GRFQConfigs.selector.addtocart_selector).attr('style','display:none !important');
          if(GRFQConfigs.show_atc=='0' || GRFQConfigs.show_atc=='1') $(GRFQConfigs.selector.addtocart_selector).attr('style','display:none !important');
          ...
          if(is_disabled){
            if(GRFQConfigs.show_atc=='1') {
              if(GRFQConfigs.shop_url=="mueblesysillones.myshopify.com") $(GRFQConfigs.selector.addtocart_selector).attr('style','display:initial !important');
              else $(GRFQConfigs.selector.addtocart_selector).eq(0).attr('style','display:initial !important');
            }
            if(GRFQConfigs.show_price=='1') {
              if(GRFQConfigs.shop_url=="mueblesysillones.myshopify.com"||GRFQConfigs.shop_url=="the-real-fur-deal.myshopify.com"||GRFQConfigs.shop_url=="lamporia-com.myshopify.com"||GRFQConfigs.shop_url=="amazontomaldives.myshopify.com"){
                  $(GRFQConfigs.selector.price_selector).attr('style','display:initial !important');
                  $(GRFQConfigs.selector.price_selector).find($(GRFQConfigs.selector.price_selector)).attr('style','display:initial !important');
              }else {
                  $(GRFQConfigs.selector.price_selector).eq(0).attr('style','display:initial !important');
                  $(GRFQConfigs.selector.price_selector).eq(0).find($(GRFQConfigs.selector.price_selector)).attr('style','display:initial !important');
              }
...
//========================================== Running Test for serveral day . Get Option================================================================
                      ...
                      var image = '';
                        ...
                        var priceSelector = $("#productPrice,span.money,#ProductPrice span.mw-price,.price-item.price-item--sale");
                        if(priceSelector.length>0||GRFQConfigs.product.price){
                          var price = 0,variant_;
                          for(var i=0;i<GRFQConfigs.product.variants.length;i++){
                            if(GRFQConfigs.product.variants[i].id==variant_id){
                            	variant_ = GRFQConfigs.product.variants[i];
                            }
                          }
                          if(priceSelector.length>=1&&typeof options !== 'string') price = parseFloat((priceSelector.html()).replace(/\D+/g, ''));
                          else if(typeof variant_ !== 'undefined'){
                          	price = variant_.price;
                          }
...
//========================================== EDD ====================================================================================
              ...
              var index_form = 0;
              var addtocart_btn = $(GRFQConfigs.selector.addtocart_selector).first();
...
//                 }
            ...
            success: function(result){
              if(result.success == true){
                $('.rfq_form_page').html(result.html).promise().done(function( arg1 ) {
                  	// Update Quantity
                    if(GRFQConfigs.shop_url=="promoland-test.myshopify.com"||GRFQConfigs.shop_url=="foundpop.myshopify.com"){
                    	$('body').on('change','#rfq_form td .item_qty',function(){
                            ...
                            $.ajax({
                                ...
                                dataType: 'jsonp',
                                type: "GET",
                                data: {quantity:qty,quote_item_id:quote_item_id,shop_url:GRFQConfigs.shop_url},
                                success: function(result){
                                	GRFQApp.UpdateQuoteCart();
                                }
                              });
        ...
        if( GRFQConfigs.collection_enable == true ){
          ...
          $('.rfq-collection-script').each(function(){
            ...
            var rfq_btn = $(this).closest('form').find('.button.rfq-btn');
  ...
  GRFQApp.add = function(el, product_id){
    ...
    var add_data = 'product_id='+product_id+'&name='+product_name+'&image='+product_image+'&product_url='+product_url+'&option='+option+'&shop='+GRFQConfigs.shop_url;
    ...
    if(priceSelector.length>0||(GRFQCollection[product_id]&&GRFQCollection[product_id].price)){
      var price = 0;
      if(priceSelector.length==1&&typeof options !== 'string') price = parseFloat((priceSelector.html()).replace(/\D+/g, ''));
  ...
  var wrong_variant = true;
    ...
    if(options!='""'||options!=''){
    	...
    	add_data += '&option='+options.replace(re, "%26");
    ...
    $.ajax({
      ...
      success: function(response) {
        if(response.success == true){
          GRFQ_setCookie(GRFQConfigs.shop_url+"quote.id", response.quote, { expires: 7, path: '/' });
          if(GRFQConfigs.shop_url=='textile-technologies-europe-ltd.myshopify.com'||GRFQConfigs.shop_url=='dev-nassauelectrical.myshopify.com'){
            window.location.href = '//'+GRFQConfigs.shop_url+'/pages/request-for-quote';
          	return false;
          }
          $("body").append(response.html);
          $('.rfq_overlay').fadeIn();
      ...
      $.ajax({
        ...
        success: function(result){
          ...
          if(result.numberofenquiry==0){
            ...
            $(".cart-icon .count , #quoteCount,.quoteCount").css('display','none');
          }else {
        ...
        function convertNumber(num){
          ...
          if(rows.length){
              ...
              $.each(rows,function(k,row){
            	...
            	console.log(total);
              $('#rfq_form .rfq-total').text(total.formatMoney(2, ',', '.'));
          }
      }
